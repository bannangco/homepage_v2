...
export const PRODUCTION_BASE_URL_ENV = "BANNANGCO_PRODUCTION_BASE_URL";
...
export function parseCliOptions(
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (argument === "--base-url") {
      ...
      cliBaseUrl = nextValue;
      index += 1;
      continue;
    }
    if (argument.startsWith("--base-url=")) {
      cliBaseUrl = argument.slice("--base-url=".length);
      if (!cliBaseUrl) {
        throw new Error("--base-url requires a value.");
      }
      continue;
    }
    throw new Error("Unknown production-check option.");
  }

  const environmentBaseUrl = environment[PRODUCTION_BASE_URL_ENV];
  if (cliBaseUrl && environmentBaseUrl) {
    throw new Error(
      `Use either --base-url or ${PRODUCTION_BASE_URL_ENV}, not both.`,
    );
  }

  return {
    baseUrl: parsePublicBaseUrl(
      cliBaseUrl || environmentBaseUrl || PRODUCTION_ORIGIN,
    ),
    help,
  };
}
